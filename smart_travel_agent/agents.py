...
class WeatherAgent:
    ...
    def run(self, *, destination: str, country: str | None = None) -> WeatherAgentResult:
        ...
        # Quick, human-friendly rule-based suitability note (keeps it deterministic).
        note_bits: list[str] = []
    ...
    def run(
        ...
        *,
        ...
        if budget_inr < total:
            ...
            # Use Gemini to suggest smart savings. (If Gemini key missing, caller handles it.)
            prompt = f"""
...
"""
        ...
        return BudgetAgentResult(breakdown_inr=breakdown, warning=warning, cheaper_alternatives=cheaper)
...
2) Day-wise itinerary in Markdown with headings:
   ...
   - Keep it realistic (no 12+ places per day).
...
"""
        ...
        # Minimal parsing: split by numbered sections if present; otherwise keep as-is.
        # This keeps code beginner-friendly and robust for college demos.
        best_time = "Suggested dates: " + (best_dates_text or "N/A")
        itinerary = text
        packing = ""
        final_rec = ""

        # Try to extract sections if the model follows the structure.
        lower = text.lower()
        if "packing" in lower and "itinerary" in lower:
            # Very forgiving splits.
            ...
            # Now split rest into itinerary, packing, final
...
# Optional: a tiny CrewAI wrapper for future extension.
# The app runs fine without this; it's included to match the requested stack.
